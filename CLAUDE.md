...
- **No new runtime deps without strong reason** — current set (from `package.json` `dependencies`): `@anthropic-ai/claude-agent-sdk`, `lucide-react`, `react`, `react-dom`, `react-markdown`, `react-syntax-highlighter`, `remark-gfm`, `zustand`.
